# Quick Start Guide

This guide will get you up and running with the Claude Code Sidebar extension in under 5 minutes.

## Prerequisites Check

Before you start, verify you have everything installed:

```bash
# Check Node.js (should be v18+)
node --version

# Check npm
npm --version

# Check Claude Code CLI
claude --version

# Check if you have build tools (macOS)
xcode-select -p
```

If any of these commands fail, install the missing prerequisites first (see README.md).

**Important for macOS users:** If you don't have Xcode Command Line Tools, install them:
```bash
xcode-select --install
```

## Installation Steps

### 1. Get the Code

```bash
git clone https://github.com/yourusername/sidebar-claude-code-vscode-extension.git
cd sidebar-claude-code-vscode-extension
```

### 2. Install & Build

```bash
npm install
npm run compile
```

### 3. Test the Extension

Open in VS Code and press F5:

```bash
code .
# Press F5 when VS Code opens
```

A new VS Code window will open with the extension running.

### 4. Use the Extension

1. Look for the Claude Code icon in the left sidebar (activity bar)
2. Click it to open a terminal in the sidebar
3. The terminal automatically runs `claude` for you
4. Start chatting with Claude in the real terminal!

## That's It!

You now have Claude Code integrated into your VS Code sidebar.

For permanent installation (so it's available in all VS Code windows), see the README.md file.

## Common Issues

**"claude: command not found"**
- Install Claude Code CLI first
- Make sure it's in your PATH

**Extension doesn't show in sidebar**
- Make sure you pressed F5 to launch the Extension Development Host
- Look for the new VS Code window that opens

**"node-pty" errors or module version mismatch**
- This is the most common issue!
- Solution: `npm run rebuild`
- If that doesn't work: `rm -rf node_modules && npm install`
- Make sure you have Xcode Command Line Tools installed (macOS)

**Terminal shows errors**
- Check the Debug Console in the original VS Code window
- Make sure Claude Code works in your regular terminal first
